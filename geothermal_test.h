...
void CoupledTH<dim>::make_grid_and_dofs() {
  ...
  for (; cell_T != endc_T; ++cell_T) {
    ...
    for (const unsigned int j : fe_values.dof_indices()) {
      ...
      if(vert_dof_map.find(vert_coords_vec) != vert_dof_map.end() && std::find(dof_appeared_Ttu.begin(), dof_appeared_Ttu.end(), local_dof_indices_T[j]) == dof_appeared_Ttu.end() && component_j==1) {
        ...
        std::pair<types::global_dof_index,std::vector<double>> vd = std::make_pair(local_dof_indices_line[j],vert_coords_vec);
  ...
  // for(auto start_point_i : start_point) {
  //   std::pair<types::global_dof_index, double> vd2 = std::make_pair(start_point_i, EquationData::g_Tb_top);
  //   dummy_map.emplace(vd2);
  // }
}
...
void CoupledTH<dim>::assemble_T_system() {
  ...
  typename DoFHandler<dim>::active_cell_iterator cell_line = dof_handler_line.begin_active(), endc_line = dof_handler_line.end();
  for (; cell_line != endc_line; ++cell_line) {
    ...
    for (unsigned int q = 0; q < n_q_points_line; ++q) {
      ...
      for (unsigned int i = 0; i < dofs_per_line; ++i) {
        ...
        double temp_old_Ttu = old_Ttu.at(found_dof_i_Ttu);
        for (unsigned int j = 0; j < dofs_per_line; ++j) {
          types::global_dof_index j_next_index = dof_dof_map_line.at(line_local_dof_indices[j]);
          std::vector<double> point_j = dof_vert_map.at(line_local_dof_indices[j]);
          std::vector<double> point_j_next_index = dof_vert_map.at(j_next_index);
          const double distance = sqrt(pow((T_quadrature_coord[0]-point_j[0]),2)+pow((T_quadrature_coord[1]-point_j[1]),2)+pow((T_quadrature_coord[2]-point_j[2]),2))
          +sqrt(pow((T_quadrature_coord[0]-point_j_next_index[0]),2)+pow((T_quadrature_coord[1]-point_j_next_index[1]),2)+pow((T_quadrature_coord[2]-point_j_next_index[2]),2))
          -sqrt(pow((point_j[0]-point_j_next_index[0]),2)+pow((point_j[1]-point_j_next_index[1]),2)+pow((point_j[2]-point_j_next_index[2]),2));
          ...
          if (distance<1e-8) {
            ...
            + EquationData::g_c_w * EquationData::A * time_step * EquationData::u * phi_i_line * (-sqrt(pow(grad_phi_j_line[0],2)+pow(grad_phi_j_line[1],2)+pow(grad_phi_j_line[2],2))) * fe_line_values.JxW(q)
          ...
          time_step * EquationData::fD * EquationData::rho * EquationData::A / (2*EquationData::d) * pow(EquationData::u,3) * phi_i_line * fe_line_values.JxW(q);
        }
      }
    }
  }
