...
void CoupledTH<dim>::assemble_T_system() {
  ...
  auto f_1 = [=](double time_t) {
    return (time_t/time_step)*(1-time_t/time_step);
  };
  auto f_2 = [=](double time_t) {
    return 1-2*time_t/time_step;
  };
  auto f_3 = [=](double time_t) {
    return 1/time_step*(1-2*time_t/time_step);
  };
  ...
  double t_1=time_step/2*(-1/sqrt(3)+1);
  double t_2=time_step/2*(1/sqrt(3)+1);
  for (; cell_line != endc_line; ++cell_line) {
    ...
    double c = temp_u * time_step / h_ele;
    double Pe = temp_u * h_ele / EquationData::k;
    double alpha = cosh(Pe/2)/sinh(Pe/2) - 2/Pe;
    double beta = c/3 - (2*alpha)/(Pe*c);
    auto f_4 = [&f_1,&f_2,alpha,beta](double time_t) {
      return 2*alpha*f_1(time_t)+beta*f_2(time_t);
    };
    ...
    for (unsigned int q = 0; q < n_q_points_line; ++q) {
      for (unsigned int i = 0; i < dofs_per_line; ++i) {
        ...
        for (unsigned int j = 0; j < dofs_per_line; ++j) {
          ...
          double W_dN_dx;
          double W_N = ((phi_i_line * phi_j_line * fe_line_values.JxW(q))*time_step/2*(16*pow(f_1(t_1),2)+16*pow(f_1(t_2),2))+(phi_j_line * fe_line_values.JxW(q))*2*time_step*pow(-1,formula_i)*(f_1(t_1)*f_4(t_1)+f_1(t_2)*f_4(t_2)));
          double W_dN_dt = ((phi_i_line * phi_j_line * fe_line_values.JxW(q))*time_step/2*(16*(f_1(t_1)*f_3(t_1)+f_1(t_2)*f_3(t_2)))+(phi_j_line * fe_line_values.JxW(q))*2*time_step*pow(-1,formula_i)*(f_3(t_1)*f_4(t_1)+f_3(t_2)*f_4(t_2)));
          double dW_dx_dN_dx = (grad_phi_i_line * grad_phi_j_line * fe_line_values.JxW(q))*time_step/2*(16*pow(f_1(t_1),2)+16*pow(f_1(t_2),2));
          double W = (phi_i_line * fe_line_values.JxW(q))*2*time_step*(f_1(t_1)+f_1(t_2)) + fe_line_values.JxW(q)*time_step/2*pow(-1,formula_i)*(f_4(t_1)+f_4(t_2));
          // mass matrix
          ...
          T_system_matrix.add(found_dof_i_Ttu, found_dof_j_Ts, (-time_step * EquationData::hz_ff * W_N));
          ...
          if (distance<1) {
            W_dN_dx = (phi_i_line * (-grad_phi_j_line_1d) * fe_line_values.JxW(q))*time_step/2*(16*pow(f_1(t_1),2)+16*pow(f_1(t_2),2)) + ((-grad_phi_j_line_1d) * fe_line_values.JxW(q))*2*time_step*pow(-1,formula_i)*(f_1(t_1)*f_4(t_1)+f_1(t_2)*f_4(t_2));
          } else {
            W_dN_dx = (phi_i_line * (grad_phi_j_line_1d) * fe_line_values.JxW(q))*time_step/2*(16*pow(f_1(t_1),2)+16*pow(f_1(t_2),2)) + ((grad_phi_j_line_1d) * fe_line_values.JxW(q))*2*time_step*pow(-1,formula_i)*(f_1(t_1)*f_4(t_1)+f_1(t_2)*f_4(t_2));
          }
          T_system_matrix.add(found_dof_i_Ttu, found_dof_j_Ttu, EquationData::g_c_w*EquationData::A*W_N+EquationData::g_c_w*EquationData::A*time_step*(W_dN_dt)+EquationData::g_c_w*EquationData::A*time_step*temp_u*W_dN_dx+EquationData::A*EquationData::k*time_step*dW_dx_dN_dx+time_step*EquationData::hz_ff*W_N);
          //T_system_rhs(found_dof_i_Ttu) += 2*(EquationData::g_c_w * EquationData::A * phi_i_line * phi_j_line * temp_old_Ttu * fe_line_values.JxW(q));
          T_system_rhs(found_dof_i_Ttu) += (EquationData::g_c_w * EquationData::A * temp_old_Ttu * W_N +
            time_step * EquationData::fD * EquationData::rho * EquationData::A / (2*EquationData::d) * pow(temp_u,3) * W);
        }
      }
    }
  }
...
void CoupledTH<dim>::output_results(LA::MPI::Vector& solution,
  ...
  const std::string pvtu_master_filename = data_out.write_vtu_with_pvtu_record(
      "outputfiles2/", filename, timestep_number, mpi_communicator);

  if (this_mpi_process == 0) {
    static std::vector<std::pair<double, std::string>> times_and_names;
    times_and_names.push_back(
        std::pair<double, std::string>(time, pvtu_master_filename));
    std::ofstream pvd_output("outputfiles2/" + var_name + "_solution.pvd");
