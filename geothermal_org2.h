...
class CoupledTH {
 ...
 private:
  ...
  std::vector<IndexSet> T_partitioning;
  ...
  std::map<types::global_dof_index, types::global_dof_index> dof_dof_map;
  // Vector<double> initial_Ttu_solution;
...
void CoupledTH<dim>::make_grid_and_dofs() {
  ...
  for (; cell != endc; ++cell) {
    ...
    for (unsigned i=0; i<dofs_per_cell; ++i) {
      types::global_dof_index vert_dof = cell->vertex_dof_index(i,0);
      Point<3> &v = cell->vertex(i);
      if(std::find(dof_appeared.begin(), dof_appeared.end(), vert_dof) == dof_appeared.end()) {
        dof_appeared.push_back(vert_dof);
        std::vector<double> vert_coords_vec; 
        vert_coords_vec.push_back(v(0)); 
        vert_coords_vec.push_back(v(1)); 
        vert_coords_vec.push_back(v(2)); 
        // pcout << "Dof:" << vert_dof << ", coords:" << v(0) << ", " << v(1) << ", "<< v(2) << std::endl;
        std::pair<std::vector<double>, types::global_dof_index> vd = std::make_pair(vert_coords_vec, vert_dof);
  ...
  std::vector<types::global_dof_index> dof_appeared2;
  for (; cell2 != endc2; ++cell2) {
    const unsigned int dofs_per_cell2 = cell2->get_fe().n_dofs_per_cell();
    for (unsigned i=0; i<dofs_per_cell2; ++i) {
      types::global_dof_index vert_dof2 = cell2->vertex_dof_index(i,0);
      Point<3> &v2 = cell2->vertex(i);
      std::vector<double> vert_coords_vec; 
      vert_coords_vec.push_back(v2(0)); 
      vert_coords_vec.push_back(v2(1)); 
      vert_coords_vec.push_back(v2(2));

      if(vert_dof_map.find(vert_coords_vec) != vert_dof_map.end() && std::find(dof_appeared2.begin(), dof_appeared2.end(), vert_dof2) == dof_appeared2.end()) {
        dof_appeared2.push_back(vert_dof2);
        types::global_dof_index found_dof = vert_dof_map.at(vert_coords_vec);
        std::pair<types::global_dof_index, types::global_dof_index> vd2 = std::make_pair(found_dof, vert_dof2);
        dof_dof_map.emplace(vd2);
        pcout << "Dof1:" << found_dof << ", Dof2:" << vert_dof2 << std::endl;
      }
    }
  }
}
